	...
	def updateData(soup, cursor,rel_name):
		...
		for row in soup.find_all('tr', {'class' : 'BdT Bdc($c-fuji-grey-c) Ta(end) Fz(s) Whs(nw)'}):
			...
			for i in range(1,len(row)):
				...
				tup = tup + "," + c.replace(',','')
			tup += ")"
			cursor.execute("insert or ignore into %s (date,open,high,low,close,adjClose,volume) values %s" % (rel_name,tup))
			print ('>insert into %s' % rel_name)

if __name__ == '__main__':
	# sqlite db connection
	SQLITE_FILE ='./database/stocks.db'
	...
	for stock in stocks:
		...
		surl = scraper.stockURL(URL,stock)
		print('headin to ' + surl)
		browser.get(surl)
		...
		response = scraper.scrollPage(browser, SCROLL_PAUSE_TIME)
		
		# parse the response
		parsed = scraper.parsePage(response)

		# update the database
		scraper.updateData(parsed, cursor, stock)
